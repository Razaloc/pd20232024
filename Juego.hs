...
-- nos dice el número de salas resueltas al terminar la partida
salasResueltas :: Partida -> Int
salasResueltas p@(m,a) = sum [ 1 | x <- (concat(matrizLista m)), x == 'O']

-- Este mensaje saltará cuando temrines la partida, informandote del total de salas que has resuelto
titulo :: Int -> String
titulo n = case n of 
    0 -> "¿¿Cero salas??"
...
-- Este mensaje saltará cuando temrines la partida, dandote un título segun el arma con el que terminasla partida
...
-- Orden superior
retoArmas :: Objeto -> [Monstruo] -> [Objeto]
retoArmas a ms = filter (/=a) (map obtenerRecompensaMonstruo listaMonstruos)
        ...
        then do
            ...
            if eleccion == 1 then do 
                ...
                if (obtenerVidaAventurero aventureroHerido) <= 0 then do
                    putStrLn "\n\tEstas muerto. FIN DEL JUEGO."
                    putStrLn (titulo (salasResueltas partida))
                    putStrLn "\n\tEstas son otras armas que podrías haber conseguido: "
                    putStrLn (show (retoArmas (obtenerMochilaAventurero aventurero) listaMonstruos))
                    nuevoJuego
                else do 
        ...
        -- postEscuentroExito -->estado de la partidadespuésdelencuentro con lucha, se modifica el estado de la sala cambia a 0
                    ...
                    if not (finalizado postEncuentroExito) then do
                        seleccionDestino postEncuentroExito
                            ...
                            putStrLn "\n\tEnhorabuena has ganado. FIN DEL JUEGO."
